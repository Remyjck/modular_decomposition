...
let path = "./graph.json"
let graph = Parsegraph.read_file_as_graph path
let graph2 = Parsegraph.read_file_as_graph path

let%test _ =
  let from_id_list list =
    Set.filter graph.nodes ~f:(fun v -> List.mem list v.id ~equal:Int.equal)
  in
  let module1 = [ 4; 5 ] in
  let module2 = [ 1; 2 ] in
