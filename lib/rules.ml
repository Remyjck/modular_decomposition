...
let is_atom tree = match tree.connective with Atom _ -> true | _ -> false
let is_prime tree = match tree.connective with Prime _ -> true | _ -> false
...
let switch_par_generic (select_node_in_prime : selector)
    (select_first_prime : selector) (select_corresponding : selector) tree =
  match tree.connective with
  | Par sub ->
        ...
        let prime_par_n =
          ...
          | Prime (idg, sub_graphs) ->
              ...
              let chosen_par =
                {
                  id = (chosen.id * 2) + 500;
                  connective = Par [ chosen; corr ];
                }
              in
              (*TODO +500 because the tree implementation requires distinct ids*)
              let new_nodes =
          ...
          let new_node =
            ...
            | Prime (_, sub1), Prime (_, sub2) ->
                Tensor
                  (Caml.List.map2
                     (fun t1 t2 ->
                       { id = t1.id + t2.id + 1; connective = Par [ t1; t2 ] })
                     sub1 sub2)
            | _ -> Par sub
          in
          { connective = new_node; id = tree.id + 1 })
