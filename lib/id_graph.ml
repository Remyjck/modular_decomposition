...
let isEdge (a, b) idg =
  ...
  List.find edges ~f:(fun (c, d) -> (a = c && b = d) || (a = d && b = c))
  <> None
...
let ullmann_find perm matA matB =
  ...
  let rec aux freeVert perm currentRow currentColumn =
    ...
    if currentRow = lastRow && isIso perm matA matB then true
    else if currentColumn = lastCol then false
...
let is_sub_iso idg1 idg2 =
  ...
  if l1 = 0 then true
  else if (*We want empty graphs to be trivial subgraphs*)
          l1 > l2 then false
  else if length_edges idg1 = 0 then true
  else
