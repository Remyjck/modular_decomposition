...
let to_vertex js_obj =
  ...
  let atom = Atom { label; pol = polarisation } in
  { connective = atom; id }
...
let make_tree_node node successors : Tree.tree =
  ...
  let connective =
    ...
    | "atom" ->
        Atom { label = Int.to_string id; pol = true }
        (*TODO could lead to edge cases*)
...
let from_vertex vertex =
  ...
  in
  `Assoc [ ("id", id); ("label", label); ("polarisation", pol) ]
...
let from_edges edge_map =
  ...
  let id_list = List.map edge_list ~f:(fun (v1, v2) -> (v1.id, v2.id)) in
...
let serialize_id_graph (id_graph : Id_graph.id_graph) =
  ...
  (*use hashmaps instead later TODO*)
  let reindexedEdges =
...
let rec serialize_tree (tree : Tree.tree) : Yojson.Basic.t =
  ...
  `Assoc [ ("node", node); ("successors", `List successors) ]
