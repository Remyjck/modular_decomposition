...
let show_graph graph =
  let () =
    Stdio.printf "Nodes: ";
    Set.iter graph.nodes ~f:(fun v -> Stdio.printf "%s " (getLabel v));
    Stdio.printf "\n"
  in
  Stdio.printf "Edges: \n";
...
let show_graph_as_tree graph =
  show_tree (Caml.Option.get (Condense.tree_from_graph graph))

let show_graph_json graph =
  to_channel Caml.stdout (Parsegraph.serialize_graph graph)
